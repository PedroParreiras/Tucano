...
RSpec.describe Account::FinderConcern do
  ...
  describe 'remote finders' do
    let!(:account) { Fabricate(:account, username: 'Alice', domain: 'tucano.social') }

    describe '.find_remote' do
      it 'returns exact match result' do
        expect(Account.find_remote('alice', 'tucano.social')).to eq(account)
      end
      ...
      it 'returns nil when username does not match' do
        expect(Account.find_remote('a_ice', 'tucano.social')).to be_nil
      end
    ...
    describe '.find_remote!' do
      ...
      it 'raises on non-matching result' do
        expect { Account.find_remote!('missing', 'tucano.host') }.to raise_error(ActiveRecord::RecordNotFound)
      end

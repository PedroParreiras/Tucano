...
module tucano::CLI
  class Domains < Base
    ...
    long_desc <<-LONG_DESC
      ...
      `https://tucano-bar.com/users/foo`, would be purged by either
      `tootctl domains purge bar.com` or `tootctl domains purge --by-uri tucano-bar.com`.
    ...
    long_desc <<-LONG_DESC
      Crawl the fediverse by using the tucano REST API endpoints that expose

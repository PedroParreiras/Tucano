...
class Admin::Metrics::Measure::TagServersMeasure < Admin::Metrics::Measure::BaseMeasure
  ...
  def perform_total_query
    tag.statuses.where('statuses.id BETWEEN ? AND ?', tucano::Snowflake.id_at(@start_at, with_random: false), tucano::Snowflake.id_at(@end_at, with_random: false)).joins(:account).count('distinct accounts.domain')
  end

  def perform_previous_total_query
    tag.statuses.where('statuses.id BETWEEN ? AND ?', tucano::Snowflake.id_at(@start_at - length_of_period, with_random: false), tucano::Snowflake.id_at(@end_at - length_of_period, with_random: false)).joins(:account).count('distinct accounts.domain')
  end
  ...
  def earliest_status_id
    tucano::Snowflake.id_at(@start_at.beginning_of_day, with_random: false)
  end

  def latest_status_id
    tucano::Snowflake.id_at(@end_at.end_of_day, with_random: false)
  end

...
class StatusesSearchService < BaseService
  def call(query, account = nil, options = {})
    tucanoOTELTracer.in_span('StatusesSearchService#call') do |span|
      @query   = query&.strip
      @account = account
      @options = options
  ...
  def convert_deprecated_options!
    ...
    if @options[:min_id]
      timestamp = tucano::Snowflake.to_time(@options[:min_id].to_i)
      syntax_options << "after:\"#{timestamp.iso8601}\""
    end

    if @options[:max_id]
      timestamp = tucano::Snowflake.to_time(@options[:max_id].to_i)
